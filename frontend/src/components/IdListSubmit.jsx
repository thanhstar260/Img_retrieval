...
const IdListSubmit = ({ listId, ClearID}) => {
  ...
  const file = new Blob([savedIds.join(",")], { type: 'text/plain' });
  ...
  return (
    <div className="border-2 fixed top-25 right-28 bg-white rounded-lg shadow-lg z-50 w-40">
      <div className="bg-slate-200 rounded-t-lg py-2 px-6 flex justify-between border-b-2">
        ...
        <button className="text-red-500 transition-all hover:text-red-900" onClick={() => ClearID("all")}>
        ...
        <div className="max-h-96 overflow-y-auto pl-4 ">
          {listId.map((item, idx) => (
            <div className="flex justify-between p-3 pr-6 border-b last:border-none">
              ...
              <button className="text-red-500 transition-all hover:text-red-900" onClick={() => ClearID(item)}>
